...
const MainPage = () => {
  ...
  const place2 = [
    ...
    },
    {
      ...
      dcntenatvnm: '이용할인',
      dtlexpln:
        '부산-김포(월 목 15%, 금 일 10%), 제주 노선 10%, 국제선 4  6%, 성수기 및 일부기간 제외, 해당항공사 접속',
      cntadr: '02-3419-7553',
      instltnnm: '에어부산(수송사)',
      hmpg: 'https://www.welfare.mil.kr',
      rowno: '64',
    ...
    },
    {
      ...
      dcntenatvnm: '이용할인',
      dtlexpln:
        '비수기 주중 최대75%, 주말, 연휴  최대40%65%, 극성수기 최대 30%, 해당업체 사이트 접속 필요',
      cntadr: '1588-3301',
      instltnnm: '제주렌트카',
      hmpg: 'https://www.welfare.mil.kr',
      rowno: '67',
  ...
  const nextButton = () => {
    if (imgCount !== place2.map((el) => el).length - 1)
      ...
      <S.BenefitInfo>
        ...
        <S.MainImg>
          <S.BenefitDiv>
            ...
            <p>
              {place2.map((el) => el.rgn)[imgCount]} /{' '}
              {place2.map((el) => el.dcntenatvnm)[imgCount]}
            </p>
            <p>
              {place2.map((el) => el.startday)[imgCount]} ~{' '}
              {place2.map((el) => el.fnshday)[imgCount]}
            </p>
        ...
        <S.BenefitP>
          {imgCount + 1}/{place2.map((el) => el).length}
        </S.BenefitP>
  ...
  return (
    <S.MainPageContainer>
      <S.MainWrapper>
        ...
        <S.SearchDiv onClick={() => navigate('/areachoice')}>
          <p>지역선택</p>
          <BsFillCaretDownFill />
        </S.SearchDiv>
