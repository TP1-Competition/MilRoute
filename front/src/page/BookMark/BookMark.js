            ...
            .then((res) => {
              axios
                ...
                .then((res) => {
                  setBookList(res.data.routes);
                  setListName(
                    res.data.routes.map((el) => el.placeNames.join(` -> `))
                  );
                });
            });
        }
            ...
            return (
              <S.InContent key={res.id}>
                <S.Data
                  ...
                  style={{
                    backgroundColor: num === idx ? '#F9CF00' : '',
                    color: num === idx ? 'white' : 'black',
                    border:
                      num === idx ? '2px solid #F9CF00' : '2px solid #DADADA',
                  }}
                >
                  <p> {listName[idx]}</p>
                  {num === idx ? (
                ...
                {num === idx ? (
                  <S.Route>
                    <S.Route2>
                      <p>{listName[idx]}</p>
                      <div>
                        <button onClick={() => reSendPath(res.id)}>
                          바로 경로 보기
                        </button>
